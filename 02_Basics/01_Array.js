...
const nextNums = [6,7,8,9,'Done'];

firstNums.push(nextNums); // Enter nextNums to the firstNum Array

console.log(firstNums); // Array inside the Array
console.log(firstNums[4]); // 5
console.log(firstNums[3][3]); // Undefined

const allNums = firstNums.concat(nextNums); // Concat return a new array by the combination of 2 or more Arrays
console.log(allNums);

const allNewNum = [...firstNums , ...nextNums, ...allNums]; // Seprad (...) return a new array by the combination of 2 or more Arrays
console.log(allNewNum);

const otherNum = [1,2,3,4,[5,6,7,8],9,[4,5,6,[3,2,1]]];
const useable_Array = otherNum.flat(Infinity)  // Flat Method is use to return all the nested arrays into a single Array
console.log(useable_Array);

console.log(Array.isArray("Asif Nawaz")); // Ask Does it's Array => False
console.log(Array.from("Asif Nawaz")); // Using Array.from convert a non Array into an Array

console.log(Array.from({name : "Asif"})); // Interesting Fact => From Doesn't convert directly an Object into an Array 1st  we have to tell him to convert Key / Values Otherwise it's return only Empty Array

let score1 = 100;
let score2 = 200;
let score3 = 300;

console.log(Array.of(score1 , score2 , score3)); // Array.of is use to convert set of elements into a new Array
